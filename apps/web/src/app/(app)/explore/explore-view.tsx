...
import type { InvestmentRoundStatus, Opportunity, OpportunityFilter } from "@rentbrown/types";
import { pluralize } from "@rentbrown/utils";

import { useOpportunities } from "../../../lib/data/hooks";
import { useScenarioKey } from "../../../lib/data/provider";
...
export function ExploreView({ initialOpportunities }: { initialOpportunities: Opportunity[] }) {
  ...
  // SSR-seeded data only applies while the mock backend matches the public
  // catalogue: default scenario and the default filter.
  const scenarioKey = useScenarioKey();
  const isDefaultFilter = status === "ALL" && query.trim() === "" && sort === "NEWEST";
  const opportunities = useOpportunities(filter, {
    initialData: scenarioKey === "default" && isDefaultFilter ? initialOpportunities : undefined,
  });
