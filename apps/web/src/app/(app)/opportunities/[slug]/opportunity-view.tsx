...
export function OpportunityView({ slug, initialOpportunity }: { slug: string; initialOpportunity: Opportunity }) {
  const router = useRouter();
  const session = useSession();
  // SSR-seeded data only applies while the mock backend matches the public
  // catalogue (default scenario).
  const scenarioKey = useScenarioKey();
  const opportunity = useOpportunity(slug, {
    initialData: scenarioKey === "default" ? initialOpportunity : undefined,
  });
